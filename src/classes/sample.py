...
from dataclasses import is_dataclass, dataclass, field, fields, replace
...
def compute_diff(
    ...
    """
    Возвращает словарь различий между двумя объектами. Различия классифицируются по операциям
    ...
    def _compute(
        ...
        """
        Рекурсивно вычисляет операции для пары значений.
        """
        ...
        if base_obj != new_obj:
            # Рекурсивно вычисляем diff для вложенных структур
            match base_obj:
                case _ if is_dataclass(base_obj):
                    # Рекурсивно обрабатываем поля dataclass'а
                    ops = {}
                    for f in fields(base_obj):
                        # Для защищённых полей на верхнем уровне пропускаем
                ...
                case set():
                    return {
                            'add': new_obj - base_obj,
                            'remove': base_obj - new_obj
                           }
                # Если оба словаря
                case dict():
                    # Словари – операции update/remove
                    update = {}
                    remove = set()
                    ...
                    return ops
                case _:
                    # Любой другой тип (пользовательский класс, не dataclass) – замена
                    return {"set": new_obj}
        return {}
...
def apply_changes(
    ...
    """
    Применяет операции к объекту obj.
    operations — результат функции compute_diff.
    """
    if not operations:
        return 
            ...
            case dict():
                ...
                if "remove" in field_ops:
                    for k in field_ops["remove"]:
                        current.pop(k, None)
            case _:
                # Для всех остальных типов (скаляры, списки, пользовательские классы)
                if "set" in field_ops:
...
class Sample:
    ...
    task_channels:Dict[str, Dict[str, Dict[str, Any]]] = field(default_factory=dict) # {stage:{task_name:{arg:value}}, ...}
    ...
    group:str = field(default="") #from_dict
    subgroup:str = field(default="") #from_dict
